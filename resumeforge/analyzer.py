...
Focado em baixa latência para evitar o Timeout de 30s do Render.
"""
...
gemini_key = GEMINI_API_KEYS[0] if isinstance(GEMINI_API_KEYS, list) and GEMINI_API_KEYS else GEMINI_API_KEYS
groq_key = GROQ_API_KEYS[0] if isinstance(GROQ_API_KEYS, list) and GROQ_API_KEYS else GROQ_API_KEYS
...
---"""
    ...
    return MatchResult(
        score=50, verdict="MEDIA",
        matching_skills=["Análise de Dados", "Excel Avançado"],
        missing_skills=[], transferable_skills=[],
        strengths=["Perfil técnico compatível"], weaknesses=[],
        suggestions=["Destaque suas principais realizações nas experiências recentes."],
...
def generate_tailored_resume(
    ...
    """Reescreve o currículo em uma única tentativa rápida sem travar o servidor."""
    resume_section = resume_text[:3500] if resume_text else ""
    if resume_data:
        resume_section = resume_data.model_dump_json()[:3500]
...
---"""
    ...
    # Fallback: Se bater no Rate Limit de 1 chave, devolve o currículo original sem quebrar o site
    if resume_data:
        return resume_data
    raise RuntimeError("Cota da chave do Gemini temporariamente atingida. Aguarde 1 minuto e tente novamente.")
...
Competências: {skills_filtradas}."""
    ...
    return (
        ...
        f"Possuo sólida experiência em análise de dados e ferramentas essenciais para a função.\n\n"
        f"Fico à disposição para uma entrevista.\n\nAtenciosamente."
    )